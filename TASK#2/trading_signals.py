...
def buy_signal3(prices, drop_threshold=-0.005, stabilization_threshold=0.001, drop_lookback=7, stabilization_lookback=1):
    """
...
def sell_signal3(prices, purchase_price, threshold=0.005, stop_loss=0.02):
    """
...
def calculate_macd(prices, short_window=5, long_window=10, signal_window=5):
    short_ema = prices.ewm(span=short_window, adjust=False, min_periods=1).mean()
    long_ema = prices.ewm(span=long_window, adjust=False, min_periods=1).mean()
    macd = short_ema - long_ema
    signal = macd.ewm(span=signal_window, adjust=False, min_periods=1).mean()
    return macd, signal

def buy_signal1(prices):
    if len(prices) < 15:
        return False

    rsi = calculate_rsi(prices, period=14).iloc[-1]
    macd, signal = calculate_macd(prices, short_window=5, long_window=10, signal_window=5)

    if rsi < 30 and macd.iloc[-1] > signal.iloc[-1] and macd.iloc[-2] <= signal.iloc[-2]:
        return True
    return False

def sell_signal1(prices, buy_price):
    if len(prices) < 15:
        return False
